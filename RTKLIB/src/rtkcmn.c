...
extern int satno(int sys, int prn)
{
    ...
    switch (sys) {
        ...
        case SYS_CMP:
            trace(4, "enter beidou %d\n", NSATGPS+NSATGLO+NSATGAL+NSATQZS+prn-MINPRNCMP+1);
            if (prn<MINPRNCMP||MAXPRNCMP<prn) {trace(4, "prn:%d, min: %d, max: %d\n", prn, MINPRNCMP, MAXPRNCMP); return 0;}
...
extern int satid2no(const char *id)
{
    ...
    switch (code) {
        ...
        default: { trace(4,"new nav sys."); return 0;}
    }
    trace(4,"sys(%d): %c%d %d\n",sys, code, prn, satno(sys,prn));
    return satno(sys,prn);
}
